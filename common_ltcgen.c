...
void
encoder_setup (int fps_num, int fps_den, enum LTC_TV_STANDARD ltc_tv, int samplerate, int userbitmode)
{
...
{
	int warn = 0;
	if ((fps_num / (double)fps_den) != 24 && (fps_num / (double)fps_den) != 25 && (fps_num / (double)fps_den) != 30 && (floor (100.0 * fps_num / (double)fps_den) != 2997)) {
...
void
set_encoder_time (double usec, long int date, int tz_minuteswest, int fps_num, int fps_den, int print)
{
...
#define FIX_SMPTE_OVERFLOW(THIS, NEXT, INC)            \
  if (bcd[(THIS)] >= (INC)) {                          \
    int ov = (int)floor ((double)bcd[(THIS)] / (INC)); \
    bcd[(THIS)] -= ov * (INC);                         \
    bcd[(NEXT)] += ov;                                 \
  }                                                    \
  if (bcd[(THIS)] < 0) {                               \
...
void
parse_string (int fps, int* bcd, char* val)
{
	int   i;
	char* buf = strdup (val);
	char* t;
